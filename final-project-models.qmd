...
train1 <- bus_df[1:233, ]
test1 <- bus_df[234:292, ]
model1 <- train(`Buses: Total Estimated Ridership` ~ ., data = train1, method = "nnet", size = c(ncol(bus_df),64,32,1))
m1RMSE <- model1$results$RMSE

train2 <- bus_df[1:320, ]
test2 <- bus_df[320:401, ]
model2 <- train(`Buses: Total Estimated Ridership` ~ ., data = train2, method = "nnet", size = c(ncol(bus_df),64,32,1))
m2RMSE <- model2$results$RMSE


train3 <- bus_df[1:408, ]
test3 <- bus_df[409:511, ]
model3 <- train(`Buses: Total Estimated Ridership` ~ ., data = train3, method = "nnet", size = c(ncol(bus_df),64,32,1))
m3RMSE <- model3$results$RMSE


train4 <- bus_df[1:496, ]
test4 <- bus_df[497:620, ]
model4 <- train(`Buses: Total Estimated Ridership` ~ ., data = train4, method = "nnet", size = c(ncol(bus_df),64,32,1))
m4RMSE <- model3$results$RMSE


train5 <- bus_df[1:584, ]
test5 <- bus_df[585:730, ]
model5 <- train(`Buses: Total Estimated Ridership` ~ ., data = train5, method = "nnet", size = c(ncol(bus_df),64,32,1))
m5RMSE <- model5$results$RMSE
...
newBusrpart_RMSE <- (sum(m1RMSE)+sum(m2RMSE)+sum(m3RMSE)+sum(m4RMSE)+sum(m5RMSE))/15
```
...
newSubrpart_RMSE <- (sum(m1RMSE)+sum(m2RMSE)+sum(m3RMSE)+sum(m4RMSE)+sum(m5RMSE))/15
```
...
df_RMSE <-
  data.frame(
    Model = c('Full Bus SVM', 'New Bus SVM',
              'Full Sub SVM','New Sub SVM',
              'Full Bus LM','New Bus LM',
              'Full Sub LM','New Sub LM',
              #'Full Bus Rpart', 'New Bus Rpart',
              #'Full Sub Rpart', 'New Sub Rpart',
              'Full Bus NNet', 'New Bus NNet',
              'Full Sub NNet', 'New Sub NNet'),
    RMSE = c(fullBusSVM_RMSE, newBusSVM_RMSE, 
             fullSubSVM_RMSE, newSubSVM_RMSE,
             fullBusLM_RMSE, newBusLM_RMSE,
             fullSubLM_RMSE, newSubLM_RMSE,
             #fullBusrpart_RMSE, newBusrpart_RMSE,
             #fullSubrpart_RMSE, newSubrpart_RMSE,
             fullBusNNet_RMSE, newBusNNet_RMSE,
             fullSubNNet_RMSE, newSubNNet_RMSE
             )
  )
df_RMSE
```
