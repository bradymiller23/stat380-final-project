...
```r
# removing temp (continuous) and conditions (similar to icon, icon is better)
sub_df3 <-  
 sub_df %>%
  select(-conditions,
         -temp)

# building full model without the variables temperature (continuous) and conditions (DOESN"T WORK WELL)
full_model_sub <- lm(`Subways: Total Estimated Ridership` ~ ., data = sub_df3)
summary(full_model_sub)


sub_df3_train <- sub_df3[-test_ind, ]
sub_df3_test <- sub_df3[test_ind, ]

# creating linear regression model for bus ridership
lm_fit <- lm(`Subways: Total Estimated Ridership` ~  ., data = sub_df3_train)


# making predictions based on the model
lm_predictions <- predict(lm_fit, newdata = sub_df3_test)
lm_predictions
...
lm_RMSE_subFULL <- rmse(sub_df3_test$`Subways: Total Estimated Ridership`, lm_predictions)
lm_RMSE_subFULL
```
###### Subway
```{r}
