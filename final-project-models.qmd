...
# This function generates the training and validation indices for each fold based on time
tsCV <- function(x, k) {
  folds <- cut(seq_along(x), breaks = k, labels = FALSE)
  lapply(unique(folds), function(i) {
    val_indices <- which(folds == i)
    train_indices <- which(folds < i)
    list(train = train_indices, test = val_indices)
  })
}
...
model <- train(`Subways: Total Estimated Ridership` ~., data = data, method = "svmRadial", trControl = ctrl)
