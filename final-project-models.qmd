...
bus_new <- bus_df %>%
  select(`Buses: Total Estimated Ridership`,`Day of Week`,`Buses: % of Comparable Pre-Pandemic Day`, temp, windspeed, snowdepth)
...
avgRMSE <- (sum(m1RMSE)+sum(m2RMSE)+sum(m3RMSE)+sum(m4RMSE)+sum(m5RMSE))/15
avgRMSE
```
...
df_RMSE <-
  data.frame(
    Model = c('Full Bus Model', 'Variable Selection Bus Model', 'Full Subway Model', 'Variable Selection Subway Model'),
    RMSE = c(341047.4, 133594.3, 737898.9, 290746.7)
  )
df_RMSE
```
